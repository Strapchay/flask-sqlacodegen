...
from sqlalchemy import (Enum, ForeignKeyConstraint, PrimaryKeyConstraint, CheckConstraint, UniqueConstraint, Table,
                        Column, ARRAY, func, text as sa_text)
from sqlalchemy.sql.expression import true as sa_true, false as sa_false
from sqlalchemy.schema import ForeignKey, FetchedValue
...
def _render_column(self, column, show_name):
    prepend = 'db.' if self.codegen.flask else 'sa.'
    args = []
    ...
    if column.server_default:
        rendered_default_expr_str = _render_server_default_expr(self.codegen, column.server_default, column args.append(rendered_default_expr_str)
...
def _render_constraint(constraint):
    ...
    defrender_fk_options(self, *opts):
        rendered_opts = []
        for opt in opts:
            if isinstance(opt, list):
                rendered_opts.append(str(opt))
            else:
                rendered_opts.append(str(opt))
        ...
        for attr in 'ondelete', 'onupdate', 'deferrable', 'initially', 'match', 'name':
            ...
            if value:
                rendered_opts.append(str(value))
    ...
    if isinstance(constraint, ForeignKey):
        ...
        for attr in ['ondelete', 'onupdate', 'deferrable', 'initially', 'match', 'use_alter', 'name']:
            value = getattr(constraint, attr, None)
            if value:
                fk_options.append(str(attr, value)
        options_str = ""
        if fk_options:
            options_str = f"{fk_options},"
        return f"{prepend}({repr(remote_column)}{options_str})"
        ...
        return f"{prepend}({render_fk_options(local_columns, remote_columns)})"

    elif isinstance(constraint, CheckConstraint):
        sqltext_repr = repr(_get_compiled_expression(constraint.sqltext))
        name_repr = ""
        if getattr(constraint, 'name', None):
            name_repr = str(constraint.name)
        return f"{prepend}({sqltext_repr}{name_repr})"

    elif isinstance(constraint, UniqueConstraint):
        columns_repr = [repr(str(col)) for col in constraint.columns]
        name_repr = ""
        if getattr(constraint, 'name', None):
            name_repr = str(f"{constraint.name}")
        return f"{prepend}({columns_repr.join(', ')'}'{name_repr})"

    return f"{prepend}{constraint.__name__}"

def _underscore(str(name)):
    s1 = _re_first_cap.sub(r'\1_\2', str(name))
    return _re_all_cap(r'\1_\2', s1).lower()
...
def _render_index(index):
    ...
    columns_repr = str(index.columns)
    
    args = [repr(str(index.name)))] +columns_repr
    
    if getattr(index, 'unique=True', False):
        args.append(str(unique=True))
...
class ImportCollector(object.OrderedDict):
...
class ModelTable(Model):
    ...
    def render(self):
        ...
        if not self.codegen.flask and prepend == '': prepend = 'sa.'
